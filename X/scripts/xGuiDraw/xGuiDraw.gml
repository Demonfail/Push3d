...
while (!ds_stack_empty(_itemsStack))
{
	...
	for (var i = ds_stack_pop(_indexStack); i < _size; ++i)
	{
		...
		if (ds_map_exists(_item, "surface"))
		{
			...
			var _surface = xSurfaceCheck(_surOld, _width, _height);
			...
			if (_redraw)
			{
				xGuiTriggerEvent(_item, xGuiEventRedraw());
				_item[? "redraw"] = false;
...
if (_mouseMoved || is_undefined(_hovered))
{
	...
	if (_lastUndefined || _hovered != _newHovered)
	{
		...
		{
			xGuiTriggerEvent(_hovered, xGuiEventMouseLeave());
		}

		if (_newHovered != noone)
		{
			xGuiTriggerEvent(_newHovered, xGuiEventMouseEnter());
		}
	}
	...
	if (_newHovered != noone)
	{
		xGuiTriggerEvent(_newHovered,
			xGuiEventMouseMove(_mouseDown,
	...
	&& _hovered != noone)
{
	...
	{
		xGuiTriggerEvent(_hovered,
			xGuiEventClick(_altDown, _ctrlDown, _shiftDown));
	...
	{
		xGuiTriggerEvent(_hovered,
			xGuiEventMouseWheel(_mouseWheel, _altDown, _ctrlDown, _shiftDown));
	}
}
...
if (_widgetDrag != noone)
{
	...
	if (_mouseUp == mb_left)
	{
		...
		_gui[? "dragging"] = false;
		xGuiTriggerEvent(_widgetDrag, xGuiEventDragEnd(_mouseXDrag, _mouseYDrag));
	}
}
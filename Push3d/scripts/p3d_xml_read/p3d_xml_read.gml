...
do
{
	...
	switch (_byte)
	{
		...
		case xEXmlChars.LT:
			...
			if (_token != ""
				...
				&& p3d_xml_element_get_child_count(_parentElement) == 0)
			{
				p3d_xml_element_set_value(_parentElement, p3d_xml_parse(_token));
			}

			_element = p3d_xml_element_create();
			break;
		...
		case xEXmlChars.GT:
			...
			else if (_lastByte == xEXmlChars.Slash)
			{
				...
				{
					p3d_xml_element_add_child(_parentElement, _element);
				}
			}
			else if (_isClosing)
			{
				...
				if (is_undefined(p3d_xml_element_get_value(_parentElement))
					&& p3d_xml_element_get_child_count(_parentElement) == 0)
				{
					p3d_xml_element_set_value(_parentElement, "");
				}
				_parentElement = p3d_xml_element_get_parent(_parentElement);
				_lastElement = noone;
				ds_map_destroy(_element);
				_isClosing = false;
			}
		...
		case xEXmlChars.DQ:
			if (_isString == _byte)
			{
				...
				if (_attributeName != "")
				{
					if (_element != noone)
					{
						p3d_xml_element_set_attribute(_element, _attributeName,
							p3d_xml_parse(_token));
					}
					_attributeName = "";
				}
			}
	...
	if (_isSeparator) {
		...
		{ 
			// Set element name
			if (_element != noone && p3d_xml_element_get_name(_element) == "")
			{
				p3d_xml_element_set_name(_element, _token);
			}
			else if (_lastElement != noone
					&& p3d_xml_element_get_name(_lastElement) == "") {
				p3d_xml_element_set_name(_lastElement, _token);
			}
			_token = "";
		}
	}

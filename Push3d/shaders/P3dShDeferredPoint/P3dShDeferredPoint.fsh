...
Texture2D teP3dShadowMap            : register(t4);
...
/// +---------------------------+
float2 p3d_vec3_ToCubeUv(float3 dir)
{
...
float P3dShadowMapCompare(Texture2D shadowMap, float2 texel, float2 uv, float compareZ)
{
...
{
	float2 screenUV       = p3dUnproject(IN.Vertex);
	...
	if (dist < u_vLightPos.w)
	{
		...
		if (NdotL > 0.0)
		{
			...
			float shadow = P3dShadowMapCompare(teP3dShadowMap, u_vShadowMapTexel, p3d_vec3_ToCubeUv(-lightVec), distLinear - bias);

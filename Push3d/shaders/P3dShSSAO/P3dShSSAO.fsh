...
//> of world-space. This line is also present in the p3dSsaoInit script,
//> so don't forget to comment out that one as well!
#define P3D_SSAO_WORLD_SPACE_NORMALS

//> Must be the same values as in the p3dSsaoInit script!
#define P3D_SSAO_KERNEL_SIZE 16
...
#ifdef P3D_SSAO_WORLD_SPACE_NORMALS
...
uniform vec3  u_vSampleKernel[P3D_SSAO_KERNEL_SIZE]; //< Kernel of random vectors.
uniform vec2  u_vNoiseScale;                       //< (screenWidth,screenHeight)/P3D_SSAO_NOISE_TEXTURE_SIZE
...
void main()
{
	...
	occlusion = clamp(1.0 - occlusion / float(P3D_SSAO_KERNEL_SIZE), 0.0, 1.0);

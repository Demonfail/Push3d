...
var _texAlbedo    = sprite_get_texture(P3dSprDefault, 0);
var _texNormal    = sprite_get_texture(P3dSprDefault, 1);
var _texMaterial  = sprite_get_texture(P3dSprDefault, 2);
var _texEmissive  = sprite_get_texture(P3dSprDefault, 3);

// Check surfaces
p3d_surface_check(application_surface, _screenWidth, _screenHeight);
for (var i = P3dEGBuffer.SIZE - 1; i >= 0; --i)
{
	surGBuffer[i] = p3d_surface_check(surGBuffer[i], _screenWidth, _screenHeight);
}
...
// Sun
if (instance_exists(P3dObjLightDirectional))
{
	_shader = P3dShShadowMapDirectional;
	shader_set(P3dShShadowMapDirectional);

	var _x = x;
	var _y = y;

	with (P3dObjLightDirectional)
	{
		shadowmap = p3d_surface_check(shadowmap, shadowMapRes, shadowMapRes);
...
// Point lights
if (instance_exists(P3dObjLightPoint))
{
	_shader = P3dShShadowMap;
	shader_set(_shader);
	var _uLightPos = shader_get_uniform(_shader, "u_vLightPos");
	var _uClipFar  = shader_get_uniform(_shader, "u_fClipFar");

	with (P3dObjLightPoint)
	{
		if (!surface_exists(shadowmap))
		{
			var _world = matrix_build(0, 0, 0, 0, 0, 0, 40, 40, 40);
			var _proj  = p3d_cubemap_get_projection_matrix(0.001, radius);
			var _pos   = [x, y, z];
			for (var i = 0; i < 6; ++i)
			{
				surface_set_target(p3d_cubemap_get_surface(cubemap, i));
				draw_clear(c_red);
				matrix_set(matrix_view, p3d_cubemap_get_view_matrix(i, _pos));
	...
	with (P3dObjLightPoint)
	{
		if (!surface_exists(shadowmap))
		{
			shadowmap = p3d_cubemap_to_single_surface(cubemap, shadowmap);
		}
	}
}
...
surface_set_target_ext(1, surGBuffer[P3dEGBuffer.NormalRoughness]);
surface_set_target_ext(2, surGBuffer[P3dEGBuffer.DepthMetalness]);
surface_set_target_ext(3, surGBuffer[P3dEGBuffer.EmissiveTranslucency]);
draw_clear_alpha(0, 0);

_shader = P3dShGBuffer;
shader_set(_shader);
...
var _matViewInverse = p3d_matrix_clone(_matView);
p3d_matrix_inverse(_matViewInverse);
...
surface_set_target(surGBuffer[P3dEGBuffer.AlbedoAO]);
draw_clear_alpha(0, 0);
surface_reset_target();
surface_copy(surGBuffer[P3dEGBuffer.AlbedoAO], 0, 0, application_surface);

////////////////////////////////////////////////////////////////////////////////
// SSAO
var _ssaoWidth  = _screenWidth * ssaoResolution;
var _ssaoHeight = _screenHeight * ssaoResolution;
surSsao = p3d_surface_check(surSsao, _ssaoWidth, _ssaoHeight);
surWork = p3d_surface_check(surWork, _ssaoWidth, _ssaoHeight);
p3d_ssao_draw(
	surSsao, surWork,
	surGBuffer[P3dEGBuffer.DepthMetalness], surGBuffer[P3dEGBuffer.NormalRoughness],
	_matView,
	_matProj,
	clipFar);
...
var _texSceneNormal   = surface_get_texture(surGBuffer[P3dEGBuffer.NormalRoughness]);
var _texSceneDepth    = surface_get_texture(surGBuffer[P3dEGBuffer.DepthMetalness]);
var _texSceneEmissive = surface_get_texture(surGBuffer[P3dEGBuffer.EmissiveTranslucency]);
...
draw_surface(surGBuffer[P3dEGBuffer.AlbedoAO], 0, 0);
shader_reset();

surface_set_target_ext(1, surGBuffer[P3dEGBuffer.EmissiveTranslucency]);
...
if (instance_exists(P3dObjLightDirectional))
{
	_shader = P3dShDeferredDirectional;
	shader_set(_shader);
	...
	var _inv255         = 1 / 255;
	var _surAlbedo      = surGBuffer[P3dEGBuffer.AlbedoAO];
...
if (instance_exists(P3dObjLightPoint))
{
	...
	var _inv255          = 1 / 255;
	var _texSceneAlbedo  = surface_get_texture(surGBuffer[P3dEGBuffer.AlbedoAO]);
	var _uLightPos       = shader_get_uniform(_shader, "u_vLightPos");
...
surLightBloom = p3d_surface_check(surLightBloom, _width, _height);

surface_set_target(surLightBloom);
draw_clear_alpha(0, 1);
gpu_set_blendmode_ext(bm_dest_alpha, bm_dest_alpha);
draw_surface_stretched(surGBuffer[P3dEGBuffer.EmissiveTranslucency], 0, 0, _width, _height);
gpu_set_blendmode(bm_normal);
surface_reset_target();

// TODO: Optimize light-bloom blur.
p3d_surface_blur(surLightBloom, surWork, 2);
p3d_surface_blur(surLightBloom, surWork, 4);
p3d_surface_blur(surLightBloom, surWork, 6);
...
////////////////////////////////////////////////////////////////////////////////
// Post-process
var _shader = P3dShPostProcess;
shader_set(_shader);
texture_set_stage(shader_get_sampler_index(_shader, "u_sLut"), sprite_get_texture(P3dSprColorGradingLut, 0));
